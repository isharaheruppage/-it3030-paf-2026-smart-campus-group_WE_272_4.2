...
function Navbar() {
  ...
  return (
    <aside className="w-full border-b border-white/10 bg-gradient-to-b from-[#0B1F3A] to-[#1E3A8A] text-blue-500 shadow-2xl shadow-blue-900/20 lg:sticky lg:top-[72px] lg:flex lg:min-h-[calc(100vh-72px)] lg:w-72 lg:flex-col lg:border-r lg:border-b-0 lg:self-start lg:overflow-y-auto">
      <div className="px-8 py-6">
        <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">
          <Link to="/dashboard" className="font-semibold text-white transition hover:underline">
            {currentUser?.name}
          </Link>
      ...
      <nav className="flex flex-1 flex-col gap-1 px-4 pb-6">
        {currentUser?.role === "USER" && (
          <SidebarLink to="/bookings/new" icon="add_circle" label="Create Booking" />
        )}
        {currentUser?.role === "USER" && (
        ...
        {currentUser?.role === "ADMIN" && (
          <>
            <SidebarLink to="/admin/resources" icon="inventory_2" label="Resources" />
            <SidebarLink to="/admin/bookings" icon="admin_panel_settings" label="Booking Requests" />
            <SidebarLink to="/admin/users" icon="group" label="User Management" />
          </>
        )}
      </nav>
